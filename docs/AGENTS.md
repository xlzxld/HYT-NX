...
| YXB 压线板(v2.3) | 定位图层=YXB（只定位不建模、无半径概念，同 CXK 款）；**锚点=与 CX 出线槽线共线重合的"贴合边"中点**（同板多条共线重合段取投影并集中点防拆段，容差 `YXB_COINCIDE_TOL`=0.05、重叠≥边长一半）；**横跨槽安装（实机 7/11 板实证 + 用户拍板）**：旋转角逐板自动判向=朝向 f(背离槽) − `YXB_TEMPLATE_LONGAXIS_DEG`（=0：长边指向 f 即垂直于槽横跨，**勿改 90=顺槽，实测方向全错**）；**水平槽壁补偿：贴合边沿 X 向(f=±Y，南北槽壁)的板原地自动加 180°——短端(凸耳)朝槽外、长端(板体)朝槽里；竖直槽壁(f=±X)不翻**（用户实证：竖壁 7 板对、横壁 4 板需原地 180）；无贴合边的板跳过+日志警告；标准件设置页**无任何方向字段**；26079 前跑板实图 11 板 4 朝向实证；模板原点定案在贴合长边中点（off=0）；Z 基准仍 CX_TOP；v2.3 schema=5 |
