...
def _flush_blend_allowed(embed_ok):
    """(纯逻辑, 可离线测) 方案二规则(用户定案 2026-09-10): 嵌入端未全程
    成功(倒圆失败, 或出线口删面失败/放弃) → 齐平端不倒圆保留直角, 保证
    "双端倒圆的条必然嵌入端完好"。v2.7: 输入 = 倒圆与删面的综合结果。"""
    return bool(embed_ok)
