...
# 局部坐标的方向角——放置时模板按该角旋转后 16.6 长边沿槽向落在 CX 线上、
# 板体沿"背离槽"方向与 2D 轮廓重合(2026-09-09 斜槽壁实机定案):
#   0   = 默认正确值; 旧"水平槽壁(f=±Y)自动+180°补偿"已删——它是矩阵
#         转置 bug(NX 按转置生效, 世界角=−θ)在轴向角下的掩盖补丁
#   90  = 长边转成垂直槽(横跨), 实测方向全错, 勿改
