...
# 标准件体的"锚点记录"属性: v3.5 起是 9 个数
#   「锚点x,y,z, 记时体中心x,y,z, 记时跨度顶,底, 角度」
#   反推(动态锚点): XY = 当前体中心 + (记时锚点 − 记时体中心) —— 平移跟随;
#   Z 按"哪端动了"判定位点位移: 两端没动/只有底端动(咀尖段调长) → 不动;
#   两端同动(整体平移) → 跟着走; 只有顶端动 → 随顶段走。
#   (中心会因调长移动一半, 不能作 Z 参照 —— 实机"只动咀尖 ±40 锚点漂 ±20"案。)
#   7 数(无跨度)/4 数(偏移)为旧格式, 同口径兼容(老模型)。
...
def _anchor_of(obj):
    """读体上的锚点记录("锚点x,y,z, 记时中心x,y,z, 跨度顶,底, 角度";
    旧版 7 数无跨度 / 4 数只有偏移); 没有记录/读不到返回空串。"""
    try:
