...
定位靠**主脚本记在体上的锚点**：放置时把「记时锚点 + 记时体中心 + 放置角」写进体属性 `CAD3D_ANCHOR_OFF`，替换时按 `当前锚点 = 当前体中心 + (记时锚点 − 记时体中心)` 反推。**动态锚点（v3.5）**：你把模型和标准件**平移**到模具上之后，反推出的锚点会**跟着走**，替换就在挪好之后的位置做，不会回建模时的原地；锚点在移面/回位之后才记，长度怎么调也不会漂。⚠️ 只跟平移，旋转对位不支持。同件的多个体各算一次必须重合，这既是归组依据（一个实例只放一个新件）也是正确性自检。**全程不读图纸。**
