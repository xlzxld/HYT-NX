...
7. **一键替换标准件**：`nx_std_replace_runner.py` 独立入口，指定"把哪些换成哪些"后原位替换；定位靠反推锚点（默认不读图纸），未指定的标准件保持原样。两页向导**不记任何记忆**（每次从"不替换"与出厂参数开始）。热咀替换（v3.3，照手动日记口径）：**先按锚点原位摆正——位置从放置起就正确**，再去参，在**定位点±15mm 带内**用同步建模「移动面」对齐旧件长度，移多少往哪移由**探步现场校准**（不猜符号）；移面对不齐时**位置也不偏**，只保持新件原长并在日志写明差多少。
...
| `nx_std_replace_runner.py` | 入口 | 一键替换标准件的 NX Journal 入口（v3.3），只动标准件、按"旧件→新规格"映射替换，热咀**原位摆放 + 定位点带内移面**对齐旧件长度，两页均无记忆 |
...
**热咀长度自动对齐（v3.3，照手动日记口径）**：替换热咀族（大水口/点胶口/热咀/nozzle，关键词可在 `nx_std_config.py` 的 `NOZZLE_FAMILIES` 配置）时：① 新件**先按锚点原位摆正**——定位点=旧件锚点，位置从放置那一刻就正确；② **去掉建模参数**变成哑体（你手动也是粘贴出的哑体才移的面——之前移不动、报"面不再与邻近对象相交"就是因为在 Wave 链接体上移）；③ 用同步建模「移动面」在**定位点上下各 15mm（共 `NOZZLE_KEEP_HEAD`=30）的带内**移水平面，把总长对齐旧件——比旧件短就拉长、比旧件长就缩短；④ 移多移少、往哪边移**不靠猜**：先移一小步量出"移 1mm 总长变多少"（带内含不含咀尖决定了正负，各件不同），再按斜率一步补齐，移完复测、不齐再补。**长度口径 = 主平面口径**（顶取朝上的平面里最高的、底取朝下的平面里最低的，顶上曲面小凸起不算）。**移面对不齐时位置也不会偏**（件已在正确位置，只保持新件原长，日志写明差多少）。
