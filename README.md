...
> **当前版本**：v2.13（CAD3D 流水线）/ v1.4（模具自动开框）/ v3.5（一键替换标准件）；2026-09-19 NX10 兼容批次：热咀移面长度对齐 NX10 实机全通（表达式跨版本降级）、图层自动"全开可选"（2D 参考图直接能选中）、替换第二页只留偏移/布尔/方向、`nx_probe_len.py` 取证探针
...
7. **一键替换标准件**：`nx_std_replace_runner.py` 独立入口，指定"把哪些换成哪些"后原位替换；定位靠反推锚点（默认不读图纸），未指定的标准件保持原样。两页向导**不记任何记忆**（每次从"不替换"与出厂参数开始）。热咀替换（v3.5，照手动日记口径）：**先按锚点原位摆正 → 在[底+40, 顶+40]范围内移面（范围内一切面、所有被选实体的面合成一份一起移，按探出斜率一步移到位）→ 定位点被带走时把整件点对点平移回放置点**；长度没对齐也不留位置偏差，日志写明差多少。
...
| `nx_std_replace_runner.py` | 入口 | 一键替换标准件的 NX Journal 入口（v3.5），只动标准件、按"旧件→新规格"映射替换，热咀**原位摆放 + [底+40,顶+40]范围移面 + 定位点回位**，两页均无记忆 |
...
定位靠**主脚本记在体上的锚点**：放置时把「记时锚点 + 记时体中心 + 记时跨度顶/底 + 放置角」写进体属性 `CAD3D_ANCHOR_OFF`。**动态锚点（v3.5）**：XY 按中心偏移**平移跟随**——你把模型和标准件平移到模具上后，替换就落在挪好之后的位置；**Z 按“哪端动了”判**——只有咀尖段动（手动调长度）时锚点纹丝不动（Z 不拿体会因调长移动一半的“包围盒中心”当参照物），两端同动（平移）才跟着走。⚠️ 只跟平移，旋转对位不支持。同件的多个体各算一次必须重合，这既是归组依据（一个实例只放一个新件）也是正确性自检。**全程不读图纸。**

**热咀长度自动对齐（v3.5，照手动日记口径）**：替换热咀族（大水口/点胶口/热咀/nozzle，关键词可在 `nx_std_config.py` 的 `NOZZLE_FAMILIES` 配置）时：① 新件**先按锚点原位摆正**——定位点=旧件锚点；② **移面选择范围 = 底面到顶面再整体上移 `NOZZLE_BAND_UP`（40mm）**——咀底/顶 z 是 0/100 就选 [40,140]，咀尖底部 40mm 固定不动，**范围内一切被选中的面都要移动**，所有被选实体的面合成一份、一次「移动面」一起移，按探出斜率**一步移到位**；③ 方向**探步现场校准**：先移 2mm 复测出"移 1mm 总长变多少"的斜率，再按斜率一步补齐、复测、不齐再补；④ **定位点回位**：定位点落在范围内被带走时，移完把整个新件点对点平移回放置点（先去参——提升体是 Wave 链接不许整体移动）。**长度口径 = 主平面口径**（顶取朝上的平面里最高的、底取朝下的平面里最低的，顶上曲面小凸起不算）。

**图纸里垫片一个位置都放不上时的兜底（v3.2）**：主脚本在窗口③会临时把靠 DK 定位的件（`垫片.prt`）的定位图层与半径换成热咀那套（层 `RZ`），让它照样能放上。判据是**拿垫片自己的图层与半径去图纸里筛，一个能放的位置都筛不出来**（光看 DK 层存不存在不够）。**Z 基准与布尔方式仍是垫片自己的**，且这个临时改动**不写进记忆**——换回能定位的图纸会自动恢复原参数。另：窗口②选**针阀模式**时垫片不装、窗口③也不显示，只在**建模日志**里写一行提示。
