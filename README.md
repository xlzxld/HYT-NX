...
| `nx_std_replace_runner.py` | 入口 | 一键替换标准件的 NX Journal 入口（v3.4），只动标准件、按"旧件→新规格"映射替换，热咀**原位摆放 + [底+40,顶+40]范围移面 + 定位点回位**，两页均无记忆 |
...
门禁命令定义在 `Makefile`，与 `AGENTS.md` §2 登记保持一致。lint 走 ruff，存量 321 条历史豁免条目（F401 / I001 等 NX10/12 旧版 Python 风格）按增量仅查本次改动文件，**新增代码必须 0 问题**。test 走 `--selftest`（当前 351 项断言全绿）。
