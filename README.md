...
⚠️ 体上没有这条记录（老版主脚本建的模型）会报「无标准件锚点」并原样不动 —— 重跑一次主脚本就会补上。
...
门禁命令定义在 `Makefile`，与 `AGENTS.md` §2 登记保持一致。lint 走 ruff，存量 321 条历史豁免条目（F401 / I001 等 NX10/12 旧版 Python 风格）按增量仅查本次改动文件，**新增代码必须 0 问题**。test 走 `--selftest`（当前 308 项断言全绿）。
