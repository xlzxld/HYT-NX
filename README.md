...
> **当前版本**：v2.13（CAD3D 流水线）/ v1.4（模具自动开框）/ v3.1（一键替换标准件）
...
7. **一键替换标准件**：`nx_std_replace_runner.py` 独立入口，指定"把哪些换成哪些"后原位替换；定位靠反推锚点（默认不读图纸），未指定的标准件保持原样。v3.1 起：两页向导**不再记任何记忆**（每次从"不替换"与出厂参数开始），且**热咀替换自动按旧件长度对齐新件长度**（头部 30mm 一段不动、其余平移，与手动"移动"命令同口径）。
...
| `nx_std_replace_runner.py` | 入口 | 一键替换标准件的 NX Journal 入口（v3.1），只动标准件、按"旧件→新规格"映射替换，热咀自动对齐旧件长度，两页均无记忆 |
...
**热咀长度自动对齐（v3.1）**：替换热咀族（大水口/点胶口/热咀/nozzle，关键词可在 `nx_std_config.py` 的 `NOZZLE_FAMILIES` 配置）时，新件放好后自动把总长对齐旧件（旧件长度 = 旧件标准件体的顶部到底部）：头部（顶部往下 `NOZZLE_KEEP_HEAD`，默认 30mm）不动，其余部分整体沿轴向平移——比旧件短就拉长、比旧件长就缩短，与手动用"移动"命令选 30mm 以下范围对齐长度同口径。单实体热咀没法体级平移时会打日志提示手动处理。
...
门禁命令定义在 `Makefile`，与 `AGENTS.md` §2 登记保持一致。lint 走 ruff，存量 321 条历史豁免条目（F401 / I001 等 NX10/12 旧版 Python 风格）按增量仅查本次改动文件，**新增代码必须 0 问题**。test 走 `--selftest`（当前 326 项断言全绿）。
