...
7. **一键替换标准件**：`nx_std_replace_runner.py` 独立入口，指定"把哪些换成哪些"后原位替换；定位靠反推锚点（默认不读图纸），未指定的标准件保持原样。两页向导**不记任何记忆**（每次从"不替换"与出厂参数开始）。热咀替换（v3.4，照手动日记口径）：**先按锚点原位摆正 → 在[底+40, 顶+40]范围内移面（范围内一切面、所有被选实体的面合成一份一起移，单步≤10mm，方向探步现场校准）→ 定位点被带走时把整件点对点平移回放置点**；长度没对齐也不留位置偏差，日志写明差多少。
