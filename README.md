...
**热咀长度自动对齐（v3.2）**：替换热咀族（大水口/点胶口/热咀/nozzle，关键词可在 `nx_std_config.py` 的 `NOZZLE_FAMILIES` 配置）时，新件放好后自动把总长对齐旧件：头部（顶部往下 `NOZZLE_KEEP_HEAD`，默认 30mm）不动，**这一带以下的面用同步建模「移动面」就地平移**差值——比旧件短就拉长、比旧件长就缩短，与手动"移动面"选面做法同口径。体不重建，所以头与身之间不会留缝；头身同体时该体被拉伸、分开时下面几块整体随动。**长度口径** = 该实例**所有体**的世界 Z 最高减最低（包围盒，不是名义高度），日志会逐实例打出"几个体、Z 从哪到哪 → 长多少"，并复测总长。

**图纸里垫片一个位置都放不上时的兜底（v3.2）**：主脚本在窗口③会临时把靠 DK 定位的件（`垫片.prt`）的定位图层与半径换成热咀那套（层 `RZ`），让它照样能放上。判据是**拿垫片自己的图层与半径去图纸里筛，一个能放的位置都筛不出来**（光看 DK 层存不存在不够）。**Z 基准与布尔方式仍是垫片自己的**，且这个临时改动**不写进记忆**——换回能定位的图纸会自动恢复原参数。另：窗口②选**针阀模式**时垫片不装、窗口③也不显示，只写一行提示。
