...
| 静态检查 (Lint) | 无 | ✅ 增量口径：AI 对本次改动 .py 执行 `python -m ruff check <改动文件>`；存量 322 条见豁免清单，NX10/12 兼容豁免见 ruff.toml |
| 格式化 (Format) | 无 | ✅ 对本次改动文件执行，0 差异 |
| 主干分支 | main | ✅ 禁止未经 Pull Request 直接向主干提交 |
| 已知豁免清单 | `ruff check .:全仓存量322条历史问题(F401/I001等, NX10/12旧版Python风格), 门禁按增量仅查本次改动文件, 新增代码必须0问题` | 唯一合法的基线失败白名单，新增须走 PR；按 `命令:条目描述` 逐条登记，便于机器比对与复核 |
