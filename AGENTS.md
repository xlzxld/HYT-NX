...
| 项 | 命令 / 取值 | 验证条件 |
|---|---|---|
| 技术栈 | Python（裸源码无 manifest，按 `.py` 后缀推断；Siemens NX Open 二次开发，NX 侧脚本须 NX10/12/2312 运行时，门禁跑系统 Python 3.14） | — |
| 构建 (Build) | 无 | ✅ 退出码 0，产物正常生成 |
| 测试 (Test) | `python nx_extrude_runner.py --selftest` | ✅ 全绿；已知豁免清单内且非本次引入的失败不阻断，但须列出 |
| 静态检查 (Lint) | 无 | ✅ 增量口径：AI 对本次改动 .py 执行 `python -m ruff check <改动文件>`；存量 183 条见豁免清单，NX10/12 兼容豁免见 ruff.toml |
| 格式化 (Format) | 无 | ✅ 对本次改动文件执行，0 差异 |
| 主干分支 | main | ✅ 禁止未经 Pull Request 直接向主干提交 |
| 已知豁免清单 | `ruff check .:全仓存量183条历史问题(F401为主, NX10/12旧版Python风格; 另含刻意保留的NaN自比较守卫/配置加载exec/Py3.3兼容写法), 门禁按增量仅查本次改动文件, 新增代码必须0问题` | 唯一合法的基线失败白名单，新增须走 PR；按 `命令:条目描述` 逐条登记，便于机器比对与复核 |
