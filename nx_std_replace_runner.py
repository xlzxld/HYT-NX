# -*- coding: utf-8 -*-
"""
nx_std_replace_runner.py —— 一键替换标准件（你指定换哪个）NX 日记入口 (v3.4)
=============================================================================
...
【位置怎么定 —— 读体上记的锚点，全程不读图纸】
  主脚本放置时把「记时锚点 + 记时体中心 + 放置角」记在**每个体**上。替换时：
      当前锚点 = 当前体中心 + (记时锚点 − 记时体中心)
  —— 动态锚点(v3.5)：你把模型和标准件**平移**到模具上之后，反推出的锚点
  会**跟着走**，替换就在挪好之后的位置做，不会回建模时的原地。锚点是在
  移面/回位**之后**记的，所以长度怎么调都不会漂。同一个件的多个体各算
  一次必须重合：这既是归组依据（一个实例只放一个新件），也是正确性自检。
  ⚠️ 只跟平移：旋转对位不在支持范围。
