# -*- coding: utf-8 -*-
"""
nx_std_replace_runner.py —— 一键替换标准件（你指定换哪个）NX 日记入口 (v3.5)
=============================================================================
...
【位置怎么定 —— 读体上记的锚点，全程不读图纸】
  主脚本放置时把「记时锚点 + 记时体中心 + 记时跨度顶/底 + 放置角」记在
  **每个体**上。替换时：
      XY = 当前体中心 + (记时锚点XY − 记时体中心XY)   —— 平移跟随
      Z  = 记时锚点Z + δ，δ 按"哪端动了"判：
           两端没动 / 只有底端动(咀尖段调长) → δ=0（定位点是固定端）
           两端动的一样(整体平移)            → δ=平移量（锚点跟着走）
           只有顶端动(顶段调长)              → δ=顶的位移
  —— 动态锚点(v3.5)：你把模型和标准件**平移**到模具上之后，替换就在挪好
  之后的位置做，不会回建模时的原地；**手动调热咀长度（动咀尖段）也不会
  让锚点漂**——Z 不拿"体中心"当参照物（中心会因调长移动一半，实机
  "只动咀尖 ±40 锚点漂 ±20"就是这么来的）。同一个件的多个体各算一次
  必须重合：这既是归组依据（一个实例只放一个新件），也是正确性自检。
  ⚠️ 只跟平移：旋转对位不在支持范围。

【热咀长度自动对齐（v3.4 = 完全照你手动的做法）】换热咀族(大水口/点胶口/热咀/
  nozzle, 关键词可配)时：
  ① 新件**先按锚点原位摆正**（定位点 = 旧件锚点）—— 位置从放置那一刻就正确；
  ② **移面选择范围 = 热咀底面到顶面再整体上移 40mm**（咀底/顶 z 是 0/100，
     范围就是 [40,140]）—— 咀尖底部 40mm 固定不动，**范围内被选中的一切
     面都要移动**；所有被选中实体的面合成一份、一次「移动面」一起移，
     按探出的斜率**一步移到位**；
  ③ 移多移少、往哪边移**不靠猜**：先移一小步量出"移 1mm 总长变多少"，
     再按这个斜率补齐，移完复测、不齐再补；
  ④ **定位点回位**：定位点落在范围内时会被移面带走 —— 移完把**整个新件**
     点对点平移回去（先去参），让定位点与放置点重新重合；定位点在咀尖
     底部 40mm 固定段内时不会被带走，无需回位。
  族关键词与上移量在 nx_std_config.py 的 NOZZLE_FAMILIES / NOZZLE_BAND_UP
  配置。
